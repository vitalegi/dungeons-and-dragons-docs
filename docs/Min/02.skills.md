...
| [Anello cinetico](#anello-cinetico) | close, <br>__`carica cinetica` [Ki](#ki)__ | `CHARGE d4` | Aumenta danno di un colpo senz'armi. Cariche attuali: `4/20` |
...
When the wearer makes an unarmed attack against a creature or object, they can choose to expend a number of charges up to its max to deal an additional 1d4 bludgeoning damage per charge. The charges must be expended to empower the attack before the attack is rolled.
