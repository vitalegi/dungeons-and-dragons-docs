...
| ------------------------ | ------------ | ----------------- | ------------------------ |
| :material-check-circle:{: .highlight } Bastone a una mano | n.a. | `+3` / `!d6 +3` / close | `DEX/1d6+DEX` contundente, versatile, Può attivare [Arti marziali](#arti-marziali) |
| Bastone a due mani       | n.a.        | `+2` / `!d8 +2` / close | `STR/1d8+STR` contundente, versatile |
| Attacco senz'armi        | Monaco L1   | `+3` / `!d4 +3` / close | `DEX/1d4+DEX` Può attivare [Arti marziali](#arti-marziali) |
...
| :material-close-circle:{: .forbidden } [Palmo tremante _(custom)_](#palmo-tremante-custom) | Monaco L2<br>__`CEIL(LIV/2)` [Ki](#ki)__ | `+3` / `!r 6d6` / close | Azione bonus: tocca l'obiettivo e, dal turno successivo, può applicare danni. |
